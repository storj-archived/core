...
var EmbeddedStorageAdapter = require('../../storage/adapters/embedded');
...
function FarmerInterface(options) {
  ...
  options.manager = new StorageManager(
    new EmbeddedStorageAdapter(options.storage.path, 'FarmerInterface'),
    {
      maxCapacity: utils.toNumberBytes(
        options.storage.size,
        options.storage.unit
      )
    }
  );
...
FarmerInterface.prototype._shouldSendOffer = function(contract, callback) {
  ...
  this._negotiator.call(this, contract, function(shouldNegotiate) {
    ...
    self.manager._storage.size(function(err, usedSpace) {
      if (err) {
        self._logger.error('Could not get usedSpace: %s',err.message);
        return callback(false);
      }

      var availableSpace = self.manager._options.maxCapacity - usedSpace;

      callback(
        (self._pendingOffers.length < self._options.concurrency) &&
        shouldNegotiate && self._hasFreeSpace &&
        (contract.get('data_size') <= availableSpace)
      );
    });
  });
};
