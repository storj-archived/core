...
function FileDemuxer(filePath, options) {
  var self = this;
  if (!(self instanceof FileDemuxer)) {
  ...
  // TODO for self._source pass in readableStream from blob store
  // LONG TERM TODO: Turn into a chunk store.
  self._fileSize = options.fileSize;
  self._filePosition = 0;
  self._shardSize = options.shardSize;

  // at this point node and browser converge to a readable stream _source
  self._source = filePath;
  self._currentShardIndex = 0;

  EventEmitter.call(self);
  timers.setImmediate(self._openStream.bind(self));
}
