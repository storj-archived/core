 ...
 * @param {String[]} [options.farmerBlacklist] - Reject offers from nodeIDs
 */
function OfferStream(contract, options) {
  ...
  this._opts = merge(Object.create(OfferStream.DEFAULTS), options);
...
OfferStream.prototype._read = function() {
  ...
  if (this._offersProccessed === this._opts.maxOffers) {
...
OfferStream.prototype.addOfferToQueue = function(contact, contract) {
  ...
  if (this._farmersDidOffer.indexOf(contact.nodeID) !== -1) {
    return false;
  }
  ...
  if (this._offersQueued === this._opts.maxOffers) {
    return false;
  }
