 ...
 * @license AGPL-3.0
 */
function EmbeddedStorageAdapter(storageDirPath, t) {
  if (!(this instanceof EmbeddedStorageAdapter)) {
    return new EmbeddedStorageAdapter(storageDirPath);
  }

  console.log(storageDirPath, t)

  if (t === 'FarmerInterface') {
    try {
  ...
  this._fs.once('ready', this._ready.bind(this));
}
...
EmbeddedStorageAdapter.prototype._put = function(key, item, callback) {
  ...
  this._db.get(key, { fillCache: false }, function(err, existing) {
    if (err) {
    ...
    self._db.put(key, JSON.stringify(value), {
      sync: true
    }, function(err) {
      if (err) {
        return callback(err);
