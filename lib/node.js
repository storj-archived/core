...
class StorjNode extends KademliaNode {
   ...
   * @param {string} options.dataDirectory - Path to store contracts, shards,
   * and hash table pairs
   * @param {number} options.maxStorageAllocation - Maximum bytes to store in
   * shard database
   * @param {buffer} options.ecdsaPrivateKey - ECDSA private key bytes
   */
  constructor(options) {
    mkdirp.sync(options.dataDirectory);
    super(merge(StorjNode.DEFAULTS, options, {
      storage: levelup(path.join(options.dataDirectory, 'hashtable.db'))
    }));

    this.quasar = this.plugin(quasar);
    this.spartacus = this.plugin(spartacus(options.ecdsaPrivateKey));
    this.offers = new Map();
    this.contracts = levelup(path.join(options.dataDirectory, 'contracts.db'), {
      valueEncoding: 'json'
    });
    this.shards = kfs(path.join(options.dataDirectory, 'sharddata.kfs'), {
      referenceId: this.identity,
      maxTableSize: options.maxStorageAllocation,
      sBucketOpts: { maxOpenFiles: 50 }
    });
    this.server = new ShardServer({
    ...
    this.transport.on('download', () => this.server.download(...arguments));
    this.transport.on('upload', () => this.server.upload(...arguments));
  }
  ...
  offerShardAllocation(peer, descriptor, callback) {
    this.send('OFFER', [descriptor], peer, (err, result) => {
      ...
      const contract = Contract.from(result);
   ...
   * @param {object} contract - The completed shard descriptor contract
   * @param {StorjNode~requestContractRenewalCallback} callback
   */
  ...
  /**
   * @callback StorjNode~requestContractRenewalCallback
   * @param {error|null} error
   * @param {object} contract - See {@tutorial storage-contracts}
   */
