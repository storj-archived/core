...
/**
 * Publishes a contract directly to a specific contact that will not be
 * relayed to other peers.
 * @param {Contract} contact - The contact to publish the contract
 * @param {Contract} contract - The storage contract to publish
 */
RenterInterface.prototype.publishContract = function(contact, contract) {
  ...
  const topic = contract.getTopicString();
  const nodeID = this.contact.nodeID;
  const id = uuid.v4();

  // This message doesn't need to be relayed as nodes will be selected
  // and communicated with directly, thus the TTL can be set to zero.
  const params = {
    uuid: id,
    topic: topic,
    contents: data,
  ...
  // TODO We can use CLAIM command (or similar) here that will also
  // include a transfer token to upload the data to avoid an additional
  // CONSIGNMENT network call for improved effeciency. The CLAIM
  // command can include all the details of a PUBLISH without the TTL
  // topic and publishers.
  var message = new kad.Message({
    method: 'PUBLISH',
    params: params
  });

  this.transport.send(contact, message, function(err, response) {
    if (err) {
      return callback(err);
    }
