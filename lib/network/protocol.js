...
Protocol.prototype.handleRenew = function(params, callback) {
  ...
  try {
    ...
    assert.ok(
      updatedContract.verifyExternalSignature(
  ...
  this.network.storageManager.load(dataHash, function(err, item) {
    if (err) {
      return callback(err);
    }
    ...
    updatedContract.sign('farmer', self.network.keyPair.getPrivateKey());
