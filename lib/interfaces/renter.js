...
RenterInterface.prototype.getStorageOffer = function(contract, callback) {
  ...
  setTimeout(function() {
    if (!self._pendingContracts[contract.get('data_hash')]) {
      return;
    }

    self._pendingContracts[contract.get('data_hash')](
      new Error('No storage offers were received')
    );
    delete self._pendingContracts[contract.get('data_hash')];
  }, constants.OFFER_TIMEOUT);
};
/**
