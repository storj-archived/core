...
FarmerInterface.Negotiator = function(contract, callback) {
  ...
  this.storageManager.load(contract.get('data_hash'), function(err, item) {
    ...
    var renterIDs = Object.keys(item.contracts);
    var renterHDKeys = item.contractsHDMap ? 
      Object.keys(item.contractsHDMap) : [];

    if (renterIDs.indexOf(contract.get('renter_id')) === -1 &&
        renterHDKeys.indexOf(contract.get('renter_hd_key')) === -1 ) {
