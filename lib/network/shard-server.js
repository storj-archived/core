...
function ShardServer(options) {
  ...
  // TODO pass bridges from farmer here

  this.activeTransfers = 0;
  this._nodeID = options.nodeID;
  this._bridgeClient = options.bridgeClient || new BridgeClient(); // TODO there can be multiple bridges
...
ShardServer.prototype.routeConsignment = function(req, res) {
  ...
  this._manager.load(hash, function(err, item) {
    ...
    if (typeof item.shard.write !== 'function') {
      ...
      self._bridgeClient.createExchangeReport(report); // TODO send to the bridge associated with the shard
