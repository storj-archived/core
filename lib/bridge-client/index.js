...
var graphqlTagPrint = require('graphql-tag/printer').print;
var compile = require('es6-template-strings/compile');
var models = require('storj-service-storage-models');
...
function BridgeClient(uri, options) {
  ...
  //Setup Apollo Client Middleware
  var graphqlPath = uri + '/graphql';
  var networkInterface = ApolloClient.createNetworkInterface(graphqlPath)
  ...
  networkInterface.use([{
    applyMiddleware: function(req, next) {
      ...
      opts.uri = graphqlPath;
      opts.method = 'POST';
      opts.json = merge(true, req.request);
...
BridgeClient.prototype.storeFileInBucket = function(id, token, file, cb) {
  ...
  function _createFileStagingFrame() {
    self._logger.info('Creating file staging frame.. (retry: %s)', retry);
...
BridgeClient.prototype._startTransfer = function(pointer, state, meta, done) {
  ...
  transferStatus.once('finish', function() {
    ...
    meta.exchangeReport.end(ExchangeReport.SUCCESS, 'SUCCESS');
...
BridgeClient.prototype.resolveFileFromPointers = function(pointers, mOpts, cb) {
  ...
  function _addInputToMultiplexer(pointer, onInputAdded) {
    ...
    self._createInputFromPointer(pointer, function(err, inputStream) {
      ...
      var exchangeReport = new ExchangeReport({
        reporterId: self._getReporterId(),
        clientId: self._getReporterId(),
        farmerId: pointer.farmer.nodeID
      });

      muxer.addInputSource(inputStream, pointer.hash,
                           exchangeReport, self);
...
/**
 * Add a payment processor to a user
 * @private
 * @param {String} name - Name of the payment processor (must match constants enum)
 * @param {String} data - Payment-Processor specific data
 *   `name === 'heroku'` : User's public key signed by Heroku service integration.
 *   `name === 'stripe'` : Object that has `customer` and `billingDate` property.
 *      Returned from createCustomer Stripe API.
 *       The billingDate property is the day of the month that starts the billing cycle.
 *
 * @param {Function} callback - callback
 */
BridgeClient.prototype.addPaymentProcessor = function(name, data, callback) {
  var mutationParts = compile(
    ...
    var mutationDocument = gql([mutationParts.literals, mutationParts.substitutions])
    return this._mutate({
      mutate: mutationDocument,
      variables: {
        name: name,
        data: data
      }
...
/**
 * Sends a request to the storj bridge through Apollo Client
 * @private
 * @param {String} method - HTTP verb
 * @param {String} path - Endpoint path
 * @param {Object} params - Request parameters
 * @param {Function} callback - Return the raw response stream?
 */
BridgeClient.prototype._query = function(query, variables, callback) {
  var self = this;

  self._logger.debug('Request: %s', JSON.stringify(opts));

  var queryPromise = apolloClient.query({
    query: query,
    variables: variables
  });
 ...
 * @param {Function} callback - Return the raw response stream?
 */
BridgeClient.prototype._mutate = function(mutation, variables, callback) {
  var self = this;

  self._logger.debug('Request: %s', JSON.stringify(opts));

  var mutationPromise = apolloClient.mutate({
    mutation: mutation,
    variables: variables
  });
  mutationPromise.then(callback);
