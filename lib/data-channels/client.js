...
DataChannelClient.prototype.createReadStream = function(token, hash) {
  ...
  this._client.on('message', rstream.push.bind(rstream));
  this._client.on('close', function(code, message) {
    if (code !== 1000) {
      rstream.emit('error', new Error(message));
    }
...
DataChannelClient.prototype.createWriteStream = function(token, hash) {
  ...
  var wstream = new stream.Transform({
  ...
  this._client.on('close', function(code, message) {
    if (code !== 1000) {
      return wstream.emit('error', new Error(message));
    }
