...
BridgeClient.prototype.storeFileInBucket = function(id, token, file, opts, cb) {
  ...
  var self = this;
  var fileKey = crypto.randomBytes(6).toString('hex');
...
BridgeClient.prototype._startDemuxing = function(options, cb) {
  var self = this;
  const {file, fileSize, fileName, fileKey, id} = options;
 ...
 * @param {String} options.fileKey - Key of the file in the blob store
 */
BridgeClient.prototype._handleShardStream = function(options) {
  var self = this;
  const {shardStream, index, frame, uploadState, fileKey} = options;

  var tmpName = fileKey
  var tmpFile = self._store.createWriteStream(fileKey)
