...
BridgeClient.prototype.storeFileInBucket = function(id, token, file, opts, cb) {
  ...
  cb = once(cb);
  // this will be undefined unless supplied by client
  var fileName = opts.fileName;
  // TODO: pull these cases out to functions
  switch(file.constructor) {
    case Readable:
      assert(typeof fileName === 'string', 'opts.fileName must be a string.');
      var ws = self._store.createWriteStream(fileHash);
      ...
      file.on('end', function() {
        ws.end();
        self._startDemuxing({
          file: self._store.createReadStream(fileHash),
          fileName: fileName,
          fileSize: fileSize,
          fileHash: fileHash,
          id: id,
...
BridgeClient.prototype._startDemuxing = function(options, cb) {
  var self = this;
  const {file, fileSize, fileName, fileHash, id} = options;
 ...
 * @param {String} options.fileHash - Key of the file in the blob store
 */
BridgeClient.prototype._handleShardStream = function(options) {
  var self = this;
  const {shardStream, index, frame, uploadState, fileHash} = options;

  if (uploadState.file.constructor === Readable) {
    var tmpName = fileHash
    var tmpFile = self._store.createWriteStream(fileHash)
  } 

  if (uploadState.file.constructor === String) {
  ...
  this._blacklist.toObject(function(err, blacklist){
    var meta = {
      frame: frame,
      // this stream will duplicate the shard in mem store
      //stream: self._store.createWriteStream(fileHash),
      tmpName: tmpName,
      size: 0,
      index: index,
...
BridgeClient.prototype._handleShardTmpFileFinish = function(state, meta, done) {
  ...
  if (state.file.constructor === Readable) {
    var rs = self._store.createReadStream(meta.tmpName);
  }

  if (state.file.constructor === String) {
    var rs = fs.createReadStream(meta.tmpName);
  }
 ...
 */
BridgeClient.prototype._transferShard = function(evt, shardStream, pointer, state) {
  var self = this;
  ...
  function _handleUploadError(err) {
    ...
    /* TODO: Verify retry logic works with shardStream */
    evt.emit('retry', shardStream, pointer);
  }
