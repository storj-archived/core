...
Network.prototype.leave = function(callback) {
  ...
  this._manager.close(function(err) {
    ...
    if (self.tunclient && self.tunclient.status === TunnelClient.OPEN) {
      self.tunclient.removeAllListeners();
...
Network.prototype._establishTunnel = function(tunnels, callback) {
  ...
  async.until(established, openTunnel, function(err) {
    ...
    self.tunclient = new TunnelClient(
      tunnel,
      'http://127.0.0.1:' + localAddress.port
    );

    self.tunclient.on('open', function() {
    ...
    self.tunclient.on('close', function onTunnelClosed() {
      self._logger.warn('tunnel connection closed');
      self.tunclient.removeAllListeners('error');
      self._establishTunnel(tunnels, callback);
    });

    self.tunclient.on('error', function onTunnelError(err) {
      self._logger.warn(
