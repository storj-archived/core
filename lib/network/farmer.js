...
FarmerInterface.prototype._deprecatedConfigV1 = function(options) {
  if (options.renterWhitelist) {
    this._logger.warn('config renterWhitelist is deprecated, please use bridges config instead');
  }

  if (options.bridgeUri) {
    this._logger.warn('config bridgeUri is deprecated, please use bridges config instead');
  }

  if (options.renterWhitelist && options.bridges) {
    this._logger.warn('config renterWhitelist and bridges are mutually exclusive, bridges config used only');
  }

  if (options.bridgeUri && options.bridges) {
    this._logger.warn('config bridgeUri and bridges are mutually exclusive, bridges config used only');
  }
}
