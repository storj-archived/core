...
var StorjTransport = traverse.TransportDecorator(kad.transports.HTTP);
 ...
 * @param {Boolean} options.farmer - Flag indicating desire for contracts
 */
function Network(options) {
  ...
  this._logger = new kad.Logger(this._options.loglevel, 'STORJ');
  this._contact = new kad.contacts.AddressPortContact(
    merge(this._options.contact, { nodeID: this._keypair.getNodeID() })
  );
  this._transport = new StorjTransport(this._contact, {
    logger: this._logger,
    cors: true,
    traverse: {
      none: true,
      upnp: options.upnp ? { ttl: 0, forward: this._contact.port } : false,
      stun: false,
      turn: false
    }
  });
