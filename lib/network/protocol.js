  ...
  self._contracts.load(key, function(err, localContract) {
    ...
    self._contracts.add(contract, function(err) {
      ...
      if (self._network._pendingContracts[key]) {
        ...
        callback(new Error('Contract no longer open to offers'));
      }
    });
  });
};
...
Protocol.prototype._handleAudit = function(params, callback) {
  ...
  self._shards.load(params.data_hash, function(err, shard) {
    ...
    var leaves = JSON.parse(shard.getTree());
    var proof = new Proof({ leaves: leaves, shard: buffer });
...
Protocol.prototype._handleConsign = function(params, callback) {
  ...
  self._contracts.load(params.data_hash, function(err, contract) {
    ...
    self._shards.add(shard, function(err) {
      if (err) {
        return callback(err);
      }
