...
WritableDataChannelStream.FLUSH_WAIT_TIMEOUT = 5000;
...
WritableDataChannelStream.prototype._flush = function(callback) {
  ...
  if (self._channel._client.readyState !== WebSocketClient.CLOSED) {
    ...
    setTimeout(
      self._channel._client.close.bind(self._channel._client, 1000),
      WritableDataChannelStream.FLUSH_WAIT_TIMEOUT
    );
  } else {
    callback(null);
  }
