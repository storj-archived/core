...
FileMuxer.prototype.addInputSource = function(readable,
  ...
  assert(exchangeReport, 'exchangeReport is expected');
  assert(bridgeClient && bridgeClient.createExchangeReport, 'bridgeClient is expected');

  var self = this;
  var input = readable.pipe(stream.PassThrough()).pause();

  input.once('data', function() {
  ...
  input.on('end', function() {
    ...
    var inputHash = utils.rmd160(this._hasher.digest());
    this._hasher = crypto.createHash('sha256');
    ...
    if (inputHash != hash) {
      ...
      exchangeReport.end(ExchangeReport.SUCCESS, 'TRANSFER_SUCCESS');
      bridgeClient.createExchangeReport(exchangeReport);
    }
  ...
  readable.on('error', function(err) {
    // Send the bridge a failure report
    exchangeReport.end(ExchangeReport.FAILURE, 'TRANSFER_FAILED');
    bridgeClient.createExchangeReport(exchangeReport);
