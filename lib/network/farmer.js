...
FarmerInterface.prototype.connectBridges = function() {
  ...
  async.eachSeries(this.bridges, (bridge, next) => {

    this._bridgeRequest(bridge.url, 'GET', '/contacts/' + this.contact.nodeID, (err, contact) => {
      if (err) {
        self._logger.error('Unable to contact bridge: %s', bridge.url);
        return next();
      }

      if (!contact) {
        this._connectBridge(bridge.url, (err, contact) => {
          if (err) {
            self._logger.error('Unable to connect to bridge: %s', bridge.url);
            return next();
          }
        });
      }
      ...
      if (contact.address !== this.contact.address ||
        ...
        this._updateBridgeContact(bridge.url, (err) => {
          if (err) {
            self._logger.error('Unable to connect to bridge: %s', bridge.url);
            return next();
          }
    ...
    if (err) {
      return self._logger.error('Unable to connect to bridges');
    }
    self._logger.info('Finished connecting to bridges');
  });
};
