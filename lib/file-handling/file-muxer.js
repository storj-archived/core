...
FileMuxer.prototype._read = function() {
  ...
  if (this._bytesRead === this._length) {
    var hmacHexString = this._hmac.digest('hex');
    if (this._bridgeHmacValue && this._bridgeHmacValue !== hmacHexString) {
      this.emit('error', new Error('File failed integrity check ' + this._bridgeHmacValue + ' | ' + hmacHexString));
    }
