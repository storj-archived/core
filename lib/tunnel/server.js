 ...
 * @param {Number} [options.port=4001] - Port to bind if no server provided
 * @param {Number} [options.maxTunnels=3] - Maximum number of gateways to open
 * @param {Object} [options.portRange]
 * @param {Number} [options.portRange.min=4002] - Start port to allow gateways
 * @param {Number} [options.portRange.max=4003] - Ending port to allow gateways
...
TunnelServer.prototype.close = function(callback) {
  ...
  this._server.close(function(err) {
    ...
    self._shutdownGateways(callback);
  });
};
