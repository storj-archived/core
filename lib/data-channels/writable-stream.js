...
WritableDataChannelStream.MAX_TTWA = 30000; // NB: Time To Write Acknowledgement
...
WritableDataChannelStream.prototype._write = function(chunk, enc, callback) {
  ...
  if (!this.isAuthenticated) {
    ...
    return self._channel._client.send(JSON.stringify({
      ...
      self._channel._logger.debug('channel authorized, sending  data...');
      self._sendData(chunk, callback);
    });
  }

  self._channel._logger.debug('writing data to channel');
  self._sendData(chunk, callback);
};
