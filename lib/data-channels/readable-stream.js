...
ReadableDataChannelStream.MAX_TTFB = 10000; // NB: Time To First Byte
...
ReadableDataChannelStream.prototype._read = function() {
  ...
  if (!self.isAuthenticated) {
    self._channel._logger.debug('sending authorization frame to contact');
    return self._channel._client.send(JSON.stringify({
      ...
      self._channel._logger.debug('channel authorized, waiting for data...');
...
ReadableDataChannelStream.prototype._push = function(bytes) {
  ...
  this._channel._logger.debug(
    'received %s bytes from contact, cleared ttfb',
    bytes.length
  );
  this.push(bytes);
};
