 ...
 * {@link AuditStream}
 * @constructor
 * @param {Array} leaves - Bottom leaves of the audit merkle tree
 * @param {String} challenge - The challenge data in hex to prepend to shard
 */
function ProofStream(leaves, challenge) {
  ...
  this.on('finish', this._generateProof.bind(this));
}

inherits(ProofStream, stream.Writable);
 ...
 * @private
 */
ProofStream.prototype._write = function(chunk, encoding, next) {
