...
BridgeClient.prototype._createInputFromPointer = function(pointer, callback) {
  ...
  this._logger.info('Opening data channel stream from %j', pointer.farmer);
...
BridgeClient.prototype.resolveFileFromPointers = function(pointers, callback) {
  ...
  function _addInputToMultiplexer(onInputAdded) {
    var nextPointer = pointers.shift();

    if (nextPointer) {
      return self._createInputFromPointer(nextPointer, function(inputStream) {
  ...
  muxer.on('drain', _addInputToMultiplexer.bind(null, function noop() {}));

  async.times(
    this._transferConcurrency,
    function addInputSource(n, next) {
      _addInputToMultiplexer(next);
    },
