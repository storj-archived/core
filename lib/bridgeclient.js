 ...
 * @param {KeyPair} options.logger - Logger instance
 ...
 * @private
 */
BridgeClient.prototype._validateLogger = function(logger) {
...
BridgeClient.prototype.storeFileInBucket = function(id, token, file, callback) {
  ...
  self.createFileStagingFrame(function(err, frame) {
    ...
    demuxer.on('shard', function(shardStream, index) {
      ...
      console.log('tmpName for shard: ', tmpName);

      console.log('Attempting to upload shard number ' + ( index + 1 ));
      ...
      tmpFile.on('finish', function() {
        ...
        shardFile.pipe(auditGenerator).on('finish', function() {
          ...
          console.log('Audit generation for shared done. Adding shard to file staging frame...');
          ...
          self.addShardToFileStagingFrame(frame.id, {
            ...
            if (err) {
              console.log('Failed to upload shard...');
              failedToUploadShard = true;
              return callback(err);
            }

            console.log('Shard upload completed! Pointer: ', JSON.stringify(pointer));
