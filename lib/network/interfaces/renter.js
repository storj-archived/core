...
RenterInterface.prototype.isAwaitingOffer = function(dataHash, callback) {
  utils.deprecated('RenterInterface#isAwaitingOffer', '5.0.0');
  callback(null, this.offerManager.getStream(dataHash) !== null);
};
/**
...
RenterInterface.prototype.acceptOffer = function(contact, contract, cb) {
  utils.deprecated('RenterInterface#acceptOffer', '5.0.0');
  ...
  this.isAwaitingOffer(hash, function(err, isAwaiting) {
    ...
    self.offerManager.getStream(hash).addOfferToQueue(contact, contract);
    callback(null, true);
  });
};
/**
...
RenterInterface.prototype.getStorageOffer = function(contract, bl, callback) {
  utils.deprecated('RenterInterface#getStorageOffer', '5.0.0');
  ...
  offerStream.once('data', function(offer) {
    offerStream.removeAllListeners('error');
  ...
  function _handleTimeout() {
    if (!self.offerManager.getStream(hash)) {
      return;
    }

    offerStream.emit('error', new Error('No storage offers were received'));
  }
