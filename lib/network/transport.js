...
function Transport(contact, options) {
  ...
  this._server = restify.createServer({
    name: 'storj',
    version: require('../version').protocol,
    handleUpgrades: true
  });
  ...
  this.tunnelServer = new diglet.Server({
    logger: this._log,
    proxyPortRange: options.tunnelGatewayRange,
    proxyMaxConnections: 12,
    maxProxiesAllowed: options.maxTunnels
  });
...
Transport.prototype._bindServer = function(callback) {
  ...
  self._server.post(
    '/',
    restify.bodyParser(),
    (req, res) => self._handleRPC(req, res)
  );
  self._server.post(
    '/shards/:hash',
    restify.queryParser(),
    (req, res, next) => self.shardServer.routeConsignment(req, res, next)
  );
  self._server.get(
    '/shards/:hash',
    restify.queryParser(),
    (req, res, next) => self.shardServer.routeRetrieval(req, res, next)
  );
