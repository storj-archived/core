...
BridgeClient.prototype.storeFileInBucket = function(id, token, file, opts, cb) {
  ...
  if (file instanceof Readable) {
    ...
    file.on('end', function() {
      ws.end();
      // self._startDemuxing(fileSize, fileHash, id, token, cb);
...
BridgeClient.prototype._startDemuxing = function(options, cb) {
  var self = this;
  const {fileSize, fileName, fileHash, id} = options;
  ...
  var rs = self._store.createReadStream(fileHash);
  ...
  function _createFileStagingFrame() {
    ...
    self.createFileStagingFrame(function(err, frame) {
      ...
      var demuxer = new FileDemuxer(rs, {
        shardSize: shardSize,
        fileSize: fileSize
      });
...
BridgeClient.prototype._shardUploadWorker = function(task, done) {
  ...
  task.shard.on('data', function(data) {
    ...
    //don't write the data back to the store again
    //task.meta.stream.write(data);

    // forcing shard to push null data to trigger end event below
    task.shard.push(null)
  }).resume();

  task.shard.pipe(task.meta.stream);
...
BridgeClient.prototype._startTransfer = function(pointer, state, meta, done) {
  ...
  // get another read stream from mem by the tmpName
  // we need a way to check if browser or node here
  var rs = self._store.createReadStream(meta.tmpName);

  var transferStatus = self._transferShard(
    new EventEmitter(),
    // pass the stream to transfer shard
    //meta.tmpName,
    rs,
    pointer,
    state
