...
FarmerInterface._deprecatedConfigV1 = function(options) {
  if (options.renterWhitelist) {
    self._logger.warn('config renterWhitelist is deprecated, please use bridges config instead');
  }

  if (options.bridgeUri) {
    self._logger.warn('config bridgeUri is deprecated, please use bridges config instead');
  }

  if (options.renterWhitelist && options.bridges) {
    self._logger.warn('config renterWhitelist and bridges are mutually exclusive, bridges config used only');
  }

  if (options.bridgeUri && options.bridges) {
    self._logger.warn('config bridgeUri and bridges are mutually exclusive, bridges config used only');
  }
}
...
FarmerInterface.Negotiator = function(contract, callback) {
  ...
  if (!this.bridges) {
    self._logger.debug('no bridge are connected');
    return callback(false);
  }
  var isConnected = this.bridges.has(contract.get('renter_hd_key'));
  self._logger.debug('bridge is connected: %s', isConnected);
  if (!isWhitelisted) {
    return callback(false);
  }
...
FarmerInterface.prototype.connectBridges = function() {
  ...
  // TODO start interval to keep track of the status of each
  // bridge contact.

  // TODO create bridge clients at this.bridges as a map
...
FarmerInterface.prototype._addBridgeContact = function(bridge, callback) {
  ...
  async.series([
    (next) => {
      ...
      this._bridgeRequest(bridge.url, 'POST', path, headers, body, (err, data) => {
        if (err) {
          return callback(err);
        }
        target = data.target;
    ...
    (next) => {
      ...
      this._bridgeRequest(bridge.url, 'POST', path, headers, body, next);
    }
  ], callback);
}
...
FarmerInterface.prototype._bridgeRequest = function(bridgeUrl,
                                                    method,
                                                    path,
                                                    headers,
                                                    body,
