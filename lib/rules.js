...
class Rules {
  ...
  offer(request, response, next) {
    ...
    offerStream.addOfferToQueue(request.contact, contract, (err, result) => {
      if (err) {
        return next(err);
      }
  ...
  audit(request, response, next) {
    ...
    async.mapSeries(audits, ({ hash, challenge }, done) => {
      this.node.contracts.get(`${hash}:${identity}`, (err, desc) => {
        if (err) {
  ...
  consign(request, response, next) {
    ...
    this.node.contracts.get(`${hash}:${contact[0]}`, (err, desc) => {
      if (err) {
        return next(err);
      }
