...
BridgeClient.prototype._startTransfer = function(pointer, state, meta, done) {
  ...
  // Make sure there is free memory before transferring the shard
  if (os.freemem() < meta.size) {
    setTimeout(function() {
      if (os.freemem() < meta.size) {
        state.cleanup();
        state.callback(new Error('You are too low on memory!'));
        return;
      }
...
BridgeClient.prototype._authenticate = function(opts) {
  ...
  if (this._options.keyPair) {
    ...
    self._logger.debug('Parameter for ECDSA signature: %s\\n%s\\n%s', 
                        opts.method, opts.uri, payload);
