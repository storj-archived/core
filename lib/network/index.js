...
Network.prototype._getContactList = function(callback) {
  ...
  // Use the first configured bridge
...
Network.prototype._enterOverlay = function(onConnected) {
  ...
  // TODO standard http request here
  this._getContactList({ connected: true }, function(err, seeds) {
    if (err) {
      return onConnected(
...
Network.prototype._setupTunnelClient = function(callback) {
  ...
  if (!neighbors.length) {
    ...
    // TODO standard http request here
    self.getContactList({}, function(err, result) {
      if (err) {
