...
BridgeClient.prototype.storeFileInBucket = function(id, token, file, callback) {
  var self = this;
  var uploadState = new UploadState(id, file, callback);
 ...
 * @private
 * @param {stream.Readable} stream - Shard stream
 ...
 */
BridgeClient.prototype._handleShardStream = function(stream, i, frame, state) {
  var self = this;
  var meta = {
    frame: frame,
  ...
  self._logger.info('Trying to upload shard %s index %s', meta.tmpName, meta.i);
  state.cleanQueue.push(meta.tmpName);
  ...
  stream.on('end', tmpFile.end.bind(tmpFile));
  tmpFile.on(
    'finish',
    self._handleShardTmpFileFinish.bind(self, state, meta)
  );
};
 ...
 */
BridgeClient.prototype._handleShardTmpFileFinish = function(state, meta) {
  var self = this;
  var hash = utils.rmd160(state.hasher.digest('hex'));
  ...
  shardFile.pipe(auditGenerator).on('finish', function() {
    ...
    self.addShardToFileStagingFrame(meta.frame.id, {
      ...
      self._startTransfer(pointer, state, meta);
    });
  });
};
 ...
 * @param {Object} meta - Shard metadata reference
 */
BridgeClient.prototype._startTransfer = function(pointer, state, meta) {
  var self = this;
  var transferStatus = self._transferShard(meta.tmpName, pointer);
  ...
  transferStatus.on('retry', function() {
    if (transferStatus._eventsCount < 3) {
      ...
      self._transferShard(meta.tmpName, pointer);
    } else {
      state.callback(
  ...
  transferStatus.on('finish', self._handleShardTransferComplete.bind(
    self,
    state,
    meta.frame
  ));
 ...
 * @param {Object} frame - Frame object returned from bridge
 */
BridgeClient.prototype._handleShardTransferComplete = function(state, frame) {
  var self = this;
