...
Network.prototype._verifySignature = function(options, callback) {
  ...
  var publicKeyResult = this._recoverPublicKey(options, signedmsg);
  if (!publicKeyResult[0]) {
    return callback(publicKeyResult[1]);
  }
  this._pubkeys[options.contact.nodeID] = publicKeyResult[1];

  if (!signedmsg.verify(options.address, options.signature)) {
    return callback(new Error('Signature verification failed'));
  }

  if (!this._verifyHDKeyContact(options.contact, publicKeyResult[1])) {
...
Network.prototype._recoverPublicKey = function(options, signedmsg) {
...
Network.prototype._verifyHDKeyContact = function(contact, publicKey) {
  if (contact.hdNodeKey) {
    var hdKey = HDKey.fromExtendedKey(contact.hdNodeKey);
    var nodeKey = hdKey.deriveChild(contact.hdNodeIndex);

    if (nodeKey.publicKey.compare(publicKey.toBuffer()) !== 0) {
      return true;
