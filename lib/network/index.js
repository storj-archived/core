...
Network.prototype._enterOverlay = function(callback) {
  ...
  this._logger.info('resolving seeds from %s', this._bridge);
...
Network.prototype._setupTunnelClient = function(callback) {
  ...
  function _discoverIfReachable() {
    ...
    self._requestProbe(neighbor, function(err, result) {
      if (err || result.error) {
  ...
  if (!neighbor) {
    ...
    return this._bridge.getInfo(function(err, result) {
      ...
      neighbor = self._createContact(result.info['x-network-seeds'][0]);
