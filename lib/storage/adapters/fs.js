    ...
    async.parallel([
      function getContracts(done) {
        self.__fromDirectory(contracts, function(err, result) {
          ...
          if (err) {
            return done(new Error('Failed to load the shard contracts'));
          }
      ...
      function getTrees(done) {
        self.__fromDirectory(trees, function(err, result) {
          ...
          if (err) {
            return done(new Error('Failed to load the shard audit trees'));
          }
      ...
      function getChallenges(done) {
        self.__fromDirectory(challenges, function(err, result) {
          ...
          if (err) {
            return done(new Error('Failed to load the shard audit challenges'));
          }
      ...
      function getMeta(done) {
        self.__fromDirectory(meta, function(err, result) {
          ...
          if (err) {
            return done(new Error('Failed to load the shard metadata'));
          }
...
FSStorageAdapter.prototype._keys = function(callback) {
  fs.readdir(this._sharddir, callback);
};
