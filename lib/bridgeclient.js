...
BridgeClient.prototype.resolveFileFromPointers = function(pointers, callback) {
  ...
  function _addInputToMultiplexer() {
    var nextPointer = pointers.shift();

    if (nextPointer) {
      self._createInputFromPointer(nextPointer, muxer.input.bind(muxer));
    }
  }

  muxer.on('drain', _addInputToMultiplexer);

  for (var i = 0; i < this._transferConcurrency; i++) {
    _addInputToMultiplexer();
  }
