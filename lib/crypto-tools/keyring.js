...
function KeyRing(filePath, passPhrase) {
  ...
  this._hdKeyFile = path.join(this._keyFolder, '.hd_key');
  this._hdKey = null;
 ...
 * Returns the stored {@link KeyPair} for the given id
 * @param {String} keyId - Arbitrary key ID to load
 * @param {DataCipherKeyIv} dataCipherKey - Cipher key object to set
 */
...
/**
 * Read an HD Key from disk
 * @private
 */
KeyRing.prototype._readHDKey = function() {
  if (!utils.existsSync(this._hdKeyFile)) {
    return;
  }
  var enc = fs.readFileSync(this._hdKeyFile).toString();
  this._hdKey = JSON.parse(this._decrypt(enc)).hdKey;
};

/**
 * Generate and save HD Key to disk
 */
KeyRing.prototype.generateHDKey = function() {

  if (utils.existsSync(this._hdKeyFile)) {
    throw new Error('HD Key already exists');
  }

  var mnemonic = new Mnemonic(Mnemonic.Words.ENGLISH);
  this._hdKey = mnemonic.toSeed();
  this._saveKeyToDisk('.hd_key', { hdKey: this._hdKey });
};
...
KeyRing.prototype.importMnemonic = function(mnemonic) {
  ...
  this._hdKey = Mnemonic.fromSeed(mnemonic, Mnemonic.Words.ENGLISH);
  this._saveKeyToDisk('.hd_key', { hdKey: this._hdKey });
};
...
KeyRing.prototype.exportMnemonic = function() {
  Mnemonic.fromSeed(this._hdKey, Mnemonic.Words.ENGLISH)
  return mnemonic.toString();
};

/**
 * Delete HD Key from disk
 */
KeyRing.prototype.deleteHDKey = function() {
  this.del('.hd_key');
};
