...
TunnelClient.prototype._forwardResponse = function(err, res, body) {
  ...
  try {
    message = kad.Message.fromBuffer(new Buffer(body));
  } catch (err) {
    return this.emit('error', new Error('Failed to deserialize response'));
  }

  this._logger.debug('forwarding tunneled rpc response: %j', body);
  this._muxer.write({ type: 'rpc', data: message, flags: {} });
};
