...
BridgeClient.prototype.storeFileInBucket = function(id, token, file, callback) {
  ...
  self.createFileStagingFrame(function(err, frame) {
    ...
    demuxer.on('shard', function(shardStream, index) {
      ...
      self._logger.debug('Trying to upload shard %s index %s', tmpName, index);
      ...
      tmpFile.on('finish', function() {
        ...
        self._logger.debug('Hash for this shard is: %s', hash);
        ...
        shardFile.pipe(auditGenerator).on('finish', function() {
          ...
          self._logger.debug('Audit generation for shard done.');
          self._logger.debug('Adding shard to file staging frame...');
          ...
          self.addShardToFileStagingFrame(frame.id, {
            ...
            if (err) {
              self._logger.debug('Failed to upload shard...');
              failedToUploadShard = true;
              return callback(err);
            }

            var pointerStr = JSON.stringify(pointer);
            self._logger.debug('Shard upload done! Pointer: %s', pointerStr);
