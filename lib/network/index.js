 ...
 * @param {StorageManager} options.manager - Persistence management interface
 ...
 * @emits Network#ready
 * @prop {KeyPair} keypair
 * @prop {StorageManager} manager
 * @prop {kad.Node} node - The underlying DHT node
 * @prop {TriggerManager} triggers
 * @prop {BridgeClient} bridge
 * @prop {Contact} contact
 * @prop {Transport} transport
 * @prop {kad.Router} router - The underlying DHT router
 * @prop {DataChannelServer} dataChannelServer
 */
...
Network.prototype._checkOptions = function(options) {
  ...
  assert(options.manager instanceof StorageManager, 'Invalid manager supplied');
...
Network.prototype.join = function(callback) {
  ...
  if (typeof callback === 'function') {
    ...
    self.once('connected', function() {
      ...
      if (self.transport._isPublic) {
        ...
        console.log('_listenForTunnelers called')
      }
  ...
  async.series(
    [
      utils.ensureNtpClockIsSynchronized, // TODO: Make this not fail hard
      this.manager.open.bind(this.manager),
      this._setupTunnelClient.bind(this),
    ],
    function(err) {
      if (err) {
        console.log('205', err.message);
        return self.emit('error', err);
      }
...
Network.prototype._enterOverlay = function(callback) {
  ...
  this.bridge.getContactList({ connected: true }, function(err, seeds) {
    if (err) {
      return callback(
...
Network.prototype.leave = function(callback) {
  ...
  if (self._tunclient && self._tunclient.readyState === TunnelClient.OPEN) {
    self._tunclient.removeAllListeners();
    self._tunclient.close();
  }
  ...
  this.manager.close(function(err) {
    if (err) {
      return self.emit('error', err);
    }
...
Network.prototype._initNetworkInterface = function() {
  ...
  this.bridge = new BridgeClient(this._options.bridge, {
    logger: this._logger
  });
...
Network.prototype._listenForTunnelers = function() {
  var self = this;
  var tunserver = self.transport._tunserver;
...
Network.prototype._setupTunnelClient = function(callback) {
  ...
  if (!neighbors.length) {
    ...
    return this.bridge.getInfo(function(err, result) {
      if (err) {
...
Network.prototype._findTunnel = function(neighbors, callback) {
  ...
  this._options.tunnels = this.transport._tunserver._options.maxTunnels = 0;
...
Network.prototype._establishTunnel = function(tunnels, callback) {
  ...
  async.until(established, openTunnel, function(err) {
    ...
    self._tunclient.on('close', function onTunnelClosed() {
      self._logger.warn('tunnel connection closed');
      self._tunclient.removeAllListeners('error');
      self._establishTunnel(tunnels, tunnelWasOpened ? utils.noop : callback);
    });

    self._tunclient.on('error', function onTunnelError(err) {
      self._logger.warn(
