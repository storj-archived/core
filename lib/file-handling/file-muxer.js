...
function FileMuxer(options) {
  ...
  this._hmac = crypto.createHmac('sha512', 'the same key used to decrypt');
  this._bridgeHmacValue = options.hmacValue;
...
FileMuxer.prototype.addInputSource = function(readable,
  ...
  input.on('end', function() {
    ...
    if (inputHash !== hash) {
      ...
      // Update HMAC
      self._hmac.update(Buffer.from(inputHash, 'hex'));

      // if download is complete, emit an error if the HMAC does not match
      if (self._bytesRead === self._length) {
        var hmacHexString = self._hmac.digest('hex');
        if (self._bridgeHmacValue && self._bridgeHmacValue !== hmacHexString) {
          self.emit('error', new Error('File failed integrity check'));
        }
      }
    }
