...
function ShardServer(options) {
  ...
  assert(options.farmerInterface, 'farmerInterface is expected');
  this._farmerInterface = options.farmerInterface;
...
ShardServer.prototype._sendExchangeReport = function(bridgeUrl, report) {
  ...
  this._farmerInterface.bridgeRequest(
    bridgeUrl,'POST', '/reports/exchanges',
    headers, body, (err) => {
      if (err) {
        self._log.warn('Unable to send exchange report to bridge: %s, reason: %s', bridgeUrl, err.message);
      }
  });
}
...
ShardServer.prototype.routeConsignment = function(req, res) {
  ...
  this._manager.load(hash, function(err, item) {
    ...
    const bridge = self._farmerInterface.bridges.get(bridgeExtendedKey);
