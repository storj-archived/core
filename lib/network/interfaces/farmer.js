...
FarmerInterface.Negotiator = function(contract, callback) {
  ...
  if (this._renterWhitelist) {
    var renterIsInWhitelist = this._renterWhitelist.indexOf(
      contract.get('renter_id')
    ) !== -1;

    self._logger.debug('renter is whitelisted: %s', renterIsInWhitelist);

    if (!renterIsInWhitelist) {
...
FarmerInterface.prototype._negotiateContract = function(contract) {
  ...
  item.addContract({ nodeID: renterId }, contract.toObject());
