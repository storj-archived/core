...
BridgeClient.prototype.resolveFileFromPointers = function(pointers, callback) {
  ...
  function _addInputToMultiplexer(pointer, onInputAdded) {
    ...
    self._createInputFromPointer(pointer, function(err, inputStream) {
      if (err) {
        // TODO: get new pointer from mirror and use it instead, but for now
        // TODO: just emit an error on the muxer
 ...
 */
BridgeClient.prototype.createFileStream = function(bucket, file, callback) {
  var self = this;
  var skip = -6;
  var limit = 6;
  var resolved = false;

  function _pointersAreExhausted() {
    return resolved;
  }
  ...
  async.waterfall([
    ...
    callback(null, stream); // NB: Provide the stream as soon as it is ready
    async.until(_pointersAreExhausted, _resolveNextSlice.bind(null, queue));
  });
};
