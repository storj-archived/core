...
Network.prototype.join = function(callback) {
  ...
  this._setupTunnelClient(function(err) {
    if (err) {
      return callback(err);
    }
...
Network.prototype._enterOverlay = function(callback) {
  ...
  this._bridge.getContactList({ connected: true }, function(err, seeds) {
    ...
    self._options.seeds = seeds.map(utils.getContactURL);
...
Network.prototype._cleanRoutingTable = function() {
  this._logger.debug('cleaning bad contacts from routing table');
  ...
  this._logger.debug('dropping %s bad contacts from router', dropped.length);
...
Network.prototype._startRouterCleaner = function() {
  setInterval(
    this._cleanRoutingTable.bind(this),
