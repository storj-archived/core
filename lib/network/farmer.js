 ...
 * @param {Number} [options.allocBackoffLimit=4] - Do not send offers if more
...
function FarmerInterface(options) {
  ...
  this._allocBackoffLimit = options.allocBackoffLimit;
...
FarmerInterface.prototype._deprecatedConfigV1 = function(options) {
  ...
  if (options.offerBackoffLimit) {
    this._logger.warn('config offerBackoffLimit is deprecated, ' +
                      'please use allocBackoffLimit config instead');
  }
}
...
FarmerInterface.Negotiator = function(contract, callback) {
  ...
  var concurrentTransfer = (
    self.transport.shardServer.activeTransfers >= self._allocBackoffLimit
  );
  self._logger.debug(
    'active transfers %s is less than allocBackoffLimit %s: %s',
    self.transport.shardServer.activeTransfers,
    self._allocBackoffLimit,
    !concurrentTransfer
  );
