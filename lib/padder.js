...
Padder.prototype._flush = function(callback) {
  var remainder = this._bytesRead - Padder.DEFAULTS.mulitple;

  if (this._bytesRead < Padder.DEFAULTS.mulitple) {
    this.push(Buffer(Padder.DEFAULTS.mulitple - this._bytesRead).fill(0));
    return callback(null);
  }

  if (remainder) {
    this.push(Buffer(Padder.DEFAULTS.mulitple - remainder).fill(0));
    return callback(null);
  }
};
