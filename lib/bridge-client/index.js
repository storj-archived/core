...
BridgeClient.prototype._transferShard = function(evt, name, pointer, state) {
  ...
  shardFile.pipe(uploader).on('end', function() {
...
BridgeClient.prototype.getFilePointers = function(options, cb) {
  ...
  async.retry({
    ...
    // TODO: Do not ignore this for coverage once upgraded to async@2.x.x
    errorFilter: (e) => {
      /* istanbul ignore next */
      self._logger.warn('Request failed, reason: %s - retrying...', e.message);
      /* istanbul ignore next */
      return ['ETIMEDOUT', 'ESOCKETTIMEDOUT'].includes(e.message);
    }
  }, _request, cb);
};
...
BridgeClient.prototype._request = function(method, path, params, callback) {
  ...
  async.retry({
    ...
    errorFilter: (e) => {
      /* istanbul ignore next */
      self._logger.warn('Request failed, reason: %s', e.message);
      /* istanbul ignore next */
      return ['ETIMEDOUT', 'ESOCKETTIMEDOUT'].includes(e.message);
    }
  }, _request.bind(this), callback);
